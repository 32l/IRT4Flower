...
int main(int argc, char** argv) {
	...
	err = fopen_s(&file, "C:/Users/TK/flower1227/data/ezokanzo-0126.txt", "a");
	if(err != 0) printf("結果格納テキストファイルが見つかりません\n");

	//	画像を読み込む
	sprintf_s(buf, 100, "%s%d%s","C:/Users/TK/flower1227/ezokanzo/ezokanzo (", i, ").jpg");
	...
	//key = cvWaitKey(0);
	//if(key == 'b'){
		cv_ColorExtraction(img, paraWhite.bit, mask_img, CV_BGR2HSV, 15 , 40 ,  76, 255, 127, 255); //H S Vの順
