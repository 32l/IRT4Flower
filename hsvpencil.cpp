...
int main(int argc, char** argv) {
	...
	err = fopen_s(&file, "C:/Users/TK/flower1227/pencil.txt", "a");
	if(err != 0) printf("結果格納テキストファイルが見つかりません\n");

	//	画像を読み込む
	//sprintf_s(buf, 100, "%s%d%s","C:/Users/TK/flower1227/usuyuki/usuyuki (", i, ").jpg");
	...
	//画像構造体 {bit, convex, label, bit-window, convex-window, color}
	Para paraMax;


	Para paraRed = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1), cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(255, 0, 0), 0
	};

	Para paraYellow = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1), cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(255, 255, 0), 0
	};

	Para paraCyan = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1), cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(0, 255, 255), 0
	};

	Para paraBlue = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1),  cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(0, 0, 255), 0
	};

	Para paraMagenta = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1), cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(255, 0, 255), 0
	};

	Para paraWhite = {
		cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,1), cvCreateImage( cvSize(w, h),IPL_DEPTH_8U,3 ),
		cvCreateImage( cvSize(w, h),IPL_DEPTH_16S,1), "Result-bit","Result-convex", CV_RGB(0, 0, 0), 0
	};
	...
	if(key == 'b'){
		...
		cv_ColorExtraction(img, paraRed.bit, mask_img, CV_BGR2HSV, 165, 15, 80, 255, 0, 255);
		cv_ColorExtraction(img, paraYellow.bit, mask_img, CV_BGR2HSV, 15, 40, 80, 255, 0, 255);
		cv_ColorExtraction(img, paraCyan.bit, mask_img, CV_BGR2HSV, 75, 105, 80, 255, 0, 255);
		cv_ColorExtraction(img, paraBlue.bit, mask_img, CV_BGR2HSV, 105, 135, 80, 255, 0, 255);
		cv_ColorExtraction(img, paraMagenta.bit, mask_img, CV_BGR2HSV, 135, 165, 80, 255, 0, 255);
		cv_ColorExtraction(img, paraWhite.bit, mask_img, CV_BGR2HSV, 0, 255, 80, 255, 127, 255);
	...
	if(ratioMax.c_ratio == -1.0){
		...
		//ファイルへ書き込み
		fprintf(file,"%d %f %f %d %d %d \n", i, ratioMax.c_ratio, ratioMax.d_ratio, aveMax.h * 2, (int)aveMax.s * 100 / 255, (int)aveMax.v * 100 / 255);
		printf("c_ratio = %lf\n", ratioMax.c_ratio);
		printf("d_ratio = %1.3f\n", ratioMax.d_ratio);
	}	
	//ratioを再び-1に初期化
